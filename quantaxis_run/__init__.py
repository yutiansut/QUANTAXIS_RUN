...
class celeryconfig():
    ...
    CELERYBEAT_SCHEDULE = {
        'add':{
            'task':'quantaxis_run.run_shell_daily',
...
@app.task(bind=True)
def run_shell_daily(self, shell_cmd):
    ...
    client_qa.insert({
        ...
        'status': 'start'})
    client_joblist.insert(
        {'program': shell_cmd, 'files': filename, 'status': 'running', 'type': 'qarun_daily' ,'job_id': str(self.request.id)})
    cmd = shlex.split(shell_cmd)
    p = subprocess.Popen(
        cmd, shell=False, stdout=subprocess.PIPE, stderr=subprocess.STDOUT)
    while p.poll() is None:
